...
var Waypoints = (function () {
    ...
    Waypoints.prototype.save = function () {
        if (!this.dirty)
            return;
        fs.writeFileSync(g_rootdir + "data/" + this.user + "/waypoints.json", JSON.stringify(this.ws));
        this.dirty = false;
    };
    Waypoints.prototype.findClose = function (lat, lon) {
        var waypoints = this.ws.waypoints;
        var closest = null;
    ...
    Waypoints.getForUser = function (user) {
...
function loadUsers() {
    try {
        ...
        for (var i = 0; i < users.length; i++) {
            var x = users[i];
            if (x && typeof x == "object" && x.hasOwnProperty("user") && typeof x.user == "string" && x.hasOwnProperty("passwd") && typeof x.passwd == "string") {
                continue;
            }
